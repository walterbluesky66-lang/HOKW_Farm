...
- 主页面牧场提醒区显示最近收获批次、可收动物数量和养殖数量，并在卡片顶部提供 16h / 20h 批次收获按钮；点击后复用牧场页“我收获了”逻辑，让该组开始下一轮完整催产周期。
...
- 当前作物卡片的“我浇水了 / 好友浇水 +1 / 孙膑催熟 / 收获完成”互动按钮放在卡片顶部、农作物标题右侧；删除记录按钮也在同一顶部操作区。
...
- 在主页牧场提醒卡片顶部点击到点的 `收16h` 或 `收20h` 批次按钮，检查对应批次开始下一轮完整催产计时，另一批次不受影响；左侧不再出现重复的“牧场管理”快捷卡片。
